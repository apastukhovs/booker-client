...
import eventsModel from '../services/events';
...
export default {
  ...
  methods: {
    ...
    saveEvent() {
      ...
      .then(response => {
        calendar.getEvents();
        let time1 = format.digitTime(new Date(this.event.date + ' ' + this.event.start_time));
        let time2 =  format.digitTime(new Date(this.event.date + ' ' + this.event.end_time));
    ...
    transformFields() {
      ...
      if (this.event.date){
        if (eventCopy.start_time){
          ...
          eventCopy.start_time = format.toTimestampFormat(start_time);
        }
        if(eventCopy.end_time){
          end_time = new Date(this.event.date + ' ' + eventCopy.end_time);
          eventCopy.end_time = format.toTimestampFormat(end_time);
        }
      }
    ...
    fillFish() {
      ...
      if (calendar.selectedDate) {
        this.event.date = format.formatDate(calendar.selectedDate);
      }else{
        this.event.date = format.formatDate(new Date());
      }
